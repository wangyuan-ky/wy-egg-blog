<template>
  <div>
     <div v-if="article.id">
        ...
        <div class="bar">
          分类：<Tag @click.native="searchByCategory(article.category_id.id)" type="border" color="primary">{{article.category_id.name}}</Tag>
          标签：<Tag @click.native="searchByTag(tag.id)" v-for="tag in article.tag_id" :key="tag.id" color="primary">{{tag.tagName}}</Tag>
        </div>
        <div class="container">
